...
<div class="asset-tags <%= cssClass %>" data-facetFieldName="<%= facet.getFieldId() %>" id="<%= randomNamespace %>facet">
	...
	<ul class="<%= (showAssetCount && displayStyle.equals("cloud")) ? "tag-cloud" : "tag-list" %> nav nav-pills nav-stacked">
		...
		for (int i = 0, j = 0; i < termCollectors.size(); i++, j++) {
			...
			// FIX corrupt index issue that breaks search result
			AssetCategory curAssetCategory;
			try {
				curAssetCategory = AssetCategoryLocalServiceUtil.getAssetCategory(assetCategoryId);
			} catch (Exception e) {
				Log log = LogFactoryUtil.getLog("portal-web.docroot.html.portlet.search.facets.asset_categories_jsp");
				log.warn("Unknown asset category in search result: " + assetCategoryId);
				continue;
			}
			// FIX corrupt index issue that breaks search result
		%>
